...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var filterdSamples = samples.filter(sampleObj => sampleObj.id == sample);


    // 6. Create variables that hold the otu_ids, otu_labels, and sample_values.
    
    var otuIds = filterdSamples[0].otu_ids.map((row) => `OTU: ${row}`).slice(0,10);
    var sampleValues = filterdSamples[0].sample_values.slice(0,10).reverse(); 
    var otuLabels = filterdSamples[0].otu_labels; 
    ...
    var yticks = otuIds.sort((a,b) => a - b).reverse().slice(0,10).map(id => 'OTU' + id.toString())
